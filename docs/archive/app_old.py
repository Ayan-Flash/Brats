...
ALL_MODELS = {
    'keras_basic': {
        'path': os.path.join(CODE_DIR, 'my_model.keras'),
    ...
    'simple_cnn_code': {
        'path': os.path.join(CODE_DIR, 'simple_cnn_model.h5'),
    ...
    'brain_detector': {
        'path': os.path.join(CODE_DIR, 'brain_tumor_detector.h5'),
...
def preprocess_for_model(img_bytes, model_info):
    ...
    # Resize
    img = img.resize(model_info['input_size'])
...
def analyze():
    ...
    try:
        ...
        # Preprocess
        img_array = preprocess_for_model(img_bytes, model_info)
        ...
        if model_info['type'] == 'classification':
            ...
            mask = (probability_map >= 0.3).astype(np.uint8)
